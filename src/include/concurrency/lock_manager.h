...
class LockManager {
 ...
 private:
  ...
  std::unordered_map<table_oid_t, std::shared_ptr<LockRequestQueue>> table_lock_map_;   // 记录 table 和与其相关锁请求的映射关系。
  /** Coordination */
  std::mutex table_lock_map_latch_;

  /** Structure that holds lock requests for a given RID */
  std::unordered_map<RID, std::shared_ptr<LockRequestQueue>> row_lock_map_;             // 记录 row 和与其相关锁请求的映射关系。
  /** Coordination */
